...
class GetMethodArgsTest(unittest.TestCase):

    def test_get_method_args_no_params(self):
        def simple():
            pass
        arg_names, varargs, keywords, defaults = support.get_method_args(simple)
        self.assertEqual([], arg_names)
        self.assertEqual(None, varargs)
        self.assertEqual(None, keywords)
        self.assertEqual(None, defaults)

    def test_get_method_args_single_params(self):
        def simple(arg1):
            pass
        arg_names, varargs, keywords, defaults = support.get_method_args(simple)
        self.assertEqual(['arg1'], arg_names)
        self.assertEqual(None, varargs)
        self.assertEqual(None, keywords)
        self.assertEqual(None, defaults)

    def test_get_method_args_single_default_params(self):
        def simple(arg1, arg2='foo'):
            pass
        arg_names, varargs, keywords, defaults = support.get_method_args(simple)
        self.assertEqual(['arg1', 'arg2'], arg_names)
        self.assertEqual(None, varargs)
        self.assertEqual(None, keywords)
        self.assertEqual(('foo',), defaults)

    def test_get_method_args_varargs(self):
        def simple(arg1, arg2='foo', *args):
            pass
        arg_names, varargs, keywords, defaults = support.get_method_args(simple)
        self.assertEqual(['arg1', 'arg2'], arg_names)
        self.assertEqual('args', varargs)
        self.assertEqual(None, keywords)
        self.assertEqual(('foo',), defaults)

    def test_get_method_args_kvargs(self):
        def simple(arg1, arg2='foo', *args, **kvargs):
            pass
        arg_names, varargs, keywords, defaults = support.get_method_args(simple)
        self.assertEqual(['arg1', 'arg2'], arg_names)
        self.assertEqual('args', varargs)
        self.assertEqual('kvargs', keywords)
        self.assertEqual(('foo',), defaults)

    def test_raises_exception_if_not_method(self):
        self.assertRaises(TypeError, support.get_method_args, None)
